...
def dag_detailed_information_imosia():
    ...
    def information_on_message_actions():
        ...
        # Prepare string with information
        message_about_messages = f"{yesterday_data['unique_users'][1]} people used messages\nand sent {yesterday_data['total_messages_sent'][1]} messages in total"

        result_df = result_df[['Period', 'DAU (messages)', 'Messages sent',
                                           'Messages per user', 'Pers per user']]

        return result_df, table_with_messages, message_about_messages
    ...
    @task
    def retention_information_and_graphs():
        """
        ...
        abs_retention_graph = plot_cohort_ltv_normalized(df, 'still_there_abs', 'abs')
        organic_retention_graph = plot_cohort_ltv_normalized(df, 'still_there_organic', 'organic')

        return abs_retention_graph, organic_retention_graph
    
    @task
    def combine_message(message_about_messages, new_users_message):
        '''
    ...
    table_df_with_messages, table_with_messages, message_about_messages = information_on_message_actions()
    ...
    abs_retention_graph, organic_retention_graph = retention_information_and_graphs()
    final_message = combine_message(message_about_messages, new_users_message)
